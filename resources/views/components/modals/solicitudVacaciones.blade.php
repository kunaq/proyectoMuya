    ...
    <div class="modal fade" id="ModalFirmar" tabindex="-1" aria-labelledby="ModalFirmarLabel" aria-hidden="true">
        <div class="modal-dialog  modal-dialog-centered modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title tarjeta-vaca-foco fs-5" id="ModalFirmarLabel">Firmar convenio de adelanto de
                        vacaciones</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    Para proceder con la firma de convenio de adelanto de vacaciones la información será enviada al
                    correo <span id="correoPerMuestra"></span>, ingresar al correo mencionado para continuar con el
                    proceso.
                </div>
                <div class="modal-footer">
                    <a href="#"><button type="button" id="aceptaFirma" class="btn btn-success btnDorado"
        ...
        <div class="modal-dialog  modal-dialog-centered modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                <form action="#" id="FormSolicitudVac">
                    <div class="modal-header">
                        <h1 class="modal-title tarjeta-vaca-foco fs-5" id="ModalSolicitudLabel">Solicitud de vacaciones
                        </h1>
                        <button type="reset" id="btnDissmissModSol" class="btn-close" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            <h5 style="font-size: 17px; padding-bottom: 1rem;">Seleccione la fecha para sus vacaciones
                            </h5>
                            <div class="row">
                                ...
                                <div class="col-md-3" style="margin-bottom: 1rem;">
                                    <div class="form-group">
                                        <input type="text" class="form-control" id="datepickerIniSolVac"
                                            name="datepickerIniSolVac" required>
                                    </div>
                                </div>
                                <div class="col-md-2" style="margin-bottom: 1rem;">
                                    <div class="form-group">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <input type="text" class="form-control" id="datepickerFinSolVac"
                                            name="datepickerFinSolVac" required>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                    ...
                    <div class="modal-footer">
                        <button type="button" id="solicitaVacaciones"
                            class="btn btn-success btnDorado">Aceptar</button>
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    fechIniCalendario = (result["response"]["fch_proxima_vacaciones"] !=
                        '1900-01-01T00:00:00') ? result["response"]["fch_proxima_vacaciones"].split(
                        "T") : null;
                    fchFinCalendario = (result["response"]["fch_fin"] != '1900-01-01T00:00:00') ? result[
            ...
            $.ajax({
                ...
                success: function(respuesta) {
                    ...
                    var body = document.getElementById(
                        'bodyRegla'); //console.log('ver', respuesta['response']);
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    result['response'].forEach(element => {
                        ...
                        var tip = '';
                        var codEstado = element[
                            'dsc_estado'
                        ]; //NUEVO + VARIABLE EN FUNCION DENTRO DE filaData Y filaDataMovil
                        ...
                        var filaData = [
                            ...
                            flgFirmado,
                            flgPagado,
                            '<button style="margin-right: 5px;" class="btn btn-success btnDorado_2" data-bs-toggle="tooltip" data-bs-placement="top" ' +
                            disBtnFir + ' title="Firmar" onClick="enviaDocSoli(' + codTrabajador +
                            ',' + finFchIni + ',' + finFchFin + ',' + fchReinc + ',' + cantDias +
                            ',' + numLinea +
                            ',this)"><span class="bi bi-vector-pen"></span></button>' +
                            '<button style="margin-right: 5px;" class = "btn btn-success verdeMuya_2" data-bs-toggle="tooltip" data-bs-placement="top" title = "Descargar" ' +
                            disBtnDwn + '><span class="bi bi-download" onClick="descargaDoc(' +
                            cantDias + ',' + numLinea + ')"></span></button>' +
                            '<button class = "btn btn-secondary verdeMuya_2" data-bs-toggle="modal" data-bs-target="#ModalSolicitud" data-bs-toggle = "tooltip" data-bs-placement="top" title = "Modificar" ' +
                            disBtnEdit + ' onClick="reprograma(' + cantDias + ',' + numLinea + ',' +
                            finFchIni + ',' + finFchFin + ',' + fchReinc + ',' + codEstado +
                        ...
                        var filaDataMovil = [
                            ...
                            '<button class="btn btn-success btnDorado_2" ' + disBtnFir +
                            ' ontouchstart ="enviaDocSoli(' + codTrabajador + ',' + finFchIni +
                            ',' + finFchFin + ',' + fchReinc + ',' + cantDias + ',' + numLinea +
                            ',this)">Firma</button>' +
                            '<button class="btn btn-success verdeMuya_2"  ' + disBtnDwn +
                            ' ontouchstart="descargaDoc(' + cantDias + ',' + numLinea +
                            ')">Descarga</button>' +
                            '<button class="btn btn-secondary verdeMuya_2" data-bs-toggle="modal" data-bs-target="#ModalSolicitud" ' +
                            disBtnEdit + ' ontouchstart ="reprograma(' + cantDias + ',' + numLinea +
                            ',' + finFchIni + ',' + finFchFin + ',' + fchReinc + ',' + codEstado +
                            ')">Edita</button>' +
                            '</div>' +
                            '</td>' +
            ...
            if (pagoHaber == 'SI' && month == mes) {
                ...
                $.ajax({
                    ...
                    success: function(respuesta) {
                        ...
                        } else if (respuesta['response']['flg_coincide_grupo'] != 'SI' && respuesta[
                                'response']['ctd_coincidencia'] > parametroX) {
                            $("#overlay_load").hide();
                            Swal.fire({
                                icon: 'warning',
                            ...
                            if (reprog == 'NO') {
                                ...
                                $.ajax({
                                    ...
                                    success: function(respuesta) {
                                        ...
                                        enviaSolitudVac('@php echo(session('codTrabajador')) @endphp', fchInicio,
                                            fchFin, fchRetorno, cantDias, numSolicitud,
                                            reprog);
                                        $("#overlay_load").show();
                                        setTimeout(function() {
                                            Swal.fire({
                                ...
                                $.ajax({
                                    ...
                                    success: function(respuesta) {
                                        console.log(respuesta);
                                        enviaRechazoVacReprog('@php echo(session('codTrabajador')) @endphp',
                                            fchInicioRech, fchFinRech, fchReincRech,
                                            numLinea);

                                        //------------------------Anula solicitud anterior si esta APROBADA-------------------------
                                        var codEstado = document.getElementById("codEstado")
                                            .value;
                                        if (codEstado == '002') {
                                            dataAnu = {
                                                "cod_trabajador": '@php echo(session('codTrabajador')) @endphp',
                                        ...
                                        $.ajax({
                                            ...
                                            success: function(respuesta) {
                                                console.log(
                                                    'respuesta Insertar',
                                                    respuesta);
                                                numSolicitudIn = respuesta[
                                                    'response']['num_linea'];
                                                enviaSolitudVac(
                                                    '@php echo(session('codTrabajador')) @endphp',
                                                    fchInicio, fchFin,
                                                    fchRetorno, cantDias,
                                                    numSolicitudIn, reprog);
                                                $("#overlay_load").show();
                                                setTimeout(function() {
                                                    Swal.fire({
                                                        icon: 'success',
        ...
        function enviaSolitudVac(codTra, fchIni, fchFin, fchRinc, cantDias, numSolicitud, reprogramacion) {
            ...
            $.ajax({
                ...
                success: function(respuesta) {
                    ...
                    var actividad = 'La solicitud de vacaciones ha sido ingresada. (Inicio: ' + fchIni +
                        ', fin: ' + fchFin + ')';
                    ...
                    var fchLimiteForm = fechaActualMas8Dias.getDate().toString().padStart(2, '0') + '/' + (
                            fechaActualMas8Dias.getMonth() + 1).toString().padStart(2, '0') + '/' +
                        fechaActualMas8Dias.getFullYear();
                    ...
                    if (reprogramacion == 'NO') {
                        enviaCorreoMensaje(codSupervisor, codTra, solicitante, '1002', fchLimite, actividadSup,
                            actividadSup, numSolicitud);
                    }
        ...
        function enviaRechazoVacReprog(codTra, fchIni, fchFin, fchRinc, numSolicitud) {
            $.ajax({
                ...
                success: function(respuesta) {
                    ...
                    var fchBD = anio + '-' + mesFormateado + '-' + diaFormateado;
                    var actividad =
                        'La solicitud de vacaciones ha sido rechazada por reprogramación de vacaciones. (Inicio: ' +
                        fchIni + ', fin: ' + fchFin + ')';
                    var solicitante = "'" + '@php echo(session('nombreTrabajador')) @endphp' + "'";
                    ...
                    enviaCorreoMensaje(codSupervisor, codTra, solicitante, '1003', fchLimite, actividadJefe,
                        actividadJefe, numSolicitud);
