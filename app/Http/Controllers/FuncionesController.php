...
class FuncionesController extends Controller
{
    ...
    public function obtenerAleatorio()
    {
        ...
        try {
            $response = $client->request('GET', 'https://webapiportalplanillamuya.azurewebsites.net/api/Logueo/ObtenerClaveAleatorio/12345678911');
    ...
    public function ObtenerValidacionVacacionesPagadas(Request $request)
    {
        ...
        try {
            $response = $client->request('GET', 'https://webapiportalplanillamuya.azurewebsites.net/api/Vacaciones/ObtenerValidacionVacacionesPagadas/12345678911/' . $codTra . '/' . $numLinea);
            $statusCode = $response->getStatusCode();
            $responseData = $response->getBody()->getContents();
    ...
    public function AnularGoceVacaciones(Request $request)
    {
        ...
        try {
            $request = new \GuzzleHttp\Psr7\Request('PUT', 'https://webapiportalplanillamuya.azurewebsites.net/api/Vacaciones/AnularGoceVacaciones/12345678911', $header, $data);
            $promise = $client->sendAsync($request)->then(function ($response) {
                echo  $response->getBody();
                $code = $response->getStatusCode();
                $reason = $response->getReasonPhrase();
    ...
    public function ActualizarParametro(Request $request)
    {
        ...
        try {
            $request = new \GuzzleHttp\Psr7\Request('PUT', 'https://webapiportalplanillamuya.azurewebsites.net/api/Parametro/ActualizarParametro/12345678911', $header, $data);
            $promise = $client->sendAsync($request)->then(function ($response) {
                // echo  $response->getBody();
                $code = $response->getStatusCode();
                $reason = $response->getReasonPhrase();
    ...
    public function ListarVacacionesDias(Request $request)
    {
        ...
        try {
            $response = $client->request('GET', 'https://webapiportalplanillamuya.azurewebsites.net/api/Vacaciones/ListarVacacionesDias/12345678911/' . $numLinea);
            echo  $response->getBody();
            $code = $response->getStatusCode();
            $reason = $response->getReasonPhrase();
    ...
    public function ObtenerDiasVacacionesProyectadas(Request $request)
    {
        ...
        try {
            $response = $client->request('GET', 'https://webapiportalplanillamuya.azurewebsites.net/api/Vacaciones/ObtenerDiasVacacionesTrabajador/12345678911/' . $codTra . '/' . $fch_inicio . '/' . $fch_fin);
            echo  $response->getBody();
            $code = $response->getStatusCode();
            $reason = $response->getReasonPhrase();
